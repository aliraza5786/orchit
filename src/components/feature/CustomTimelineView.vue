...
// --- Synced Scroll between sidebar and timeline ---
let isSyncing = false;
...
const handleSidebarScroll = (e: Event) => {
  const el = e.target as HTMLElement;
  if (!isSyncing && timelineRef.value) {
    isSyncing = true;
    timelineRef.value.scrollTop = el.scrollTop;
    isSyncing = false;
  }
};
...
const timelineRange = computed(() => {
  ...
  // Buffer: -30 days from min, +90 days from max
  return {
...
onMounted(() => {
  ...
  // Initial center on today after a brief delay to ensure layout is ready
  setTimeout(goToToday, 100);
});

// Watch for range or zoom changes to re-center if necessary
...
const goToToday = () => {
  if (timelineRef.value) {
    ...
    timelineRef.value.scrollTo({
      left: offset - timelineRef.value.clientWidth / 2,
      behavior: "smooth",
    });
  }
};
...
<template>
  ...
  >
    <!-- Premium Header (Matching CustomCalendarView) -->
    <div class="flex items-center justify-between pb-2 bg-bg-surface shrink-0">
      <div class="flex items-center gap-3">
        ...
        >
          ...
          <span
            >{{ allTasks.length }} tasks · {{ packedTracks.length }} rows</span
          >
        </div>
      </div>
    ...
    <template v-else>
      ...
      <div class="flex-1 flex overflow-hidden min-h-0">
        ...
        >
          ...
          >
            <div class="flex items-center justify-between w-full">
              ...
              >
              <span
                class="text-xs font-bold px-2.5 py-1 rounded-full bg-bg-surface text-text-secondary border border-border"
              >
                {{ allTasks.length }}
              </span>
            </div>
          ...
          <!-- Sidebar rows -->
          <div
            class="flex-1 overflow-y-auto overflow-x-hidden scrollbar-thin scrollbar-thumb-border scrollbar-track-transparent"
            ref="sidebarRef"
            ...
            >
              <div class="flex items-center h-10 px-4 gap-2.5">
                ...
                <div class="flex items-center gap-2 flex-1 min-w-0">
                  <i
                    class="fa-solid fa-bolt text-primary-color text-[10px] shrink-0"
                  ></i>
                  <span
                    class="text-[11px] font-medium text-text-secondary whitespace-nowrap opacity-70 shrink-0"
                  >
                    {{ task._code }}
                  </span>
                  <span
                    class="text-[11px] font-semibold text-text-primary truncate flex-1"
                  >
                    {{ task._title }}
                  </span>
                </div>
              ...
              <div
                v-if="
                  hoveredTaskId === task._id || task._id === selectedTask?._id
                "
        ...
        <!-- ═══ TIMELINE CANVAS ═══ -->
        <div
          class="flex-1 overflow-auto scrollbar-thin scrollbar-thumb-border scrollbar-track-transparent relative bg-bg-body"
          ref="timelineRef"
        ...
        >
          ...
          >
            ...
            <div
              class="absolute top-[80px] bottom-0 w-px bg-primary-color z-[60] pointer-events-none"
            ...
            <!-- ─── HEADER ─── -->
            <div
              class="sticky top-0 z-40 bg-bg-body border-b border-border shadow-sm"
            >
              ...
              <div class="flex h-10 border-b border-border">
                ...
                >
                  <span
                    class="text-[11px] font-bold text-text-secondary uppercase tracking-wider"
                  >
                    {{ group.label }}
                  </span>
                </div>
                ...
                <div
                  ...
                  :class="{
                    ...
                    'border-l-2 border-l-border':
                      day.isMonday && zoomLevel === 'week',
                  }"
                >
                  <span
                    class="text-xs font-bold transition-colors"
                    :class="
                      day.isToday ? 'text-primary-color' : 'text-text-secondary'
                    "
                  >
                    {{ day.label }}
                  </span>
            ...
            <div class="flex-1 relative min-h-[400px]">
              ...
              <div class="absolute inset-0 pointer-events-none z-0">
                <div
                  ...
                  :style="{
                    left:
                      diffDays(day.date, timelineRange.start) * columnWidth +
                      'px',
                    width: columnWidth + 'px',
                  }"
                  :class="{
                    'bg-border/10': day.isWeekend,
                    'border-l-2 border-l-border/60 border-solid':
                      day.isMonday && zoomLevel === 'week',
                  }"
                ></div>
              </div>
                  ...
                  >
                      ...
                      <div
                        ...
                        :class="[
                          getTaskNavigation(task) === 'left'
                            ? 'left-0'
                            : 'right-0',
                          trackIdx === 0 ? 'top-full mt-2' : 'bottom-full mb-2',
                        ]"
                      >
                    ...
                    >
                      <div
                        ...
                        :style="{ backgroundColor: task._color }"
                        @mousedown="
                          handleInteractionStart($event, task, 'drag')
                        "
                      >
                        <div
                          class="handle left"
