<template>
  ...
  <div
    class="sidebar_mobile bottom-0 start-0 fixed sm:static overflow-hidden z-2 sm:z-1 sm:h-full"
  >
    <aside
      class="sm:overflow-y-auto whitespace-nowrap overflow-x-auto bg-transparent z-1 sm:min-w-[36px] sm:ps-1.5 pe-1.5 sm:max-h-full h-[50px] sm:h-full flex flex-row sm:flex-col gap-1 pt-2 sm:pt-0 pb-2 transition-all duration-200 justify-start items-start"
      :class="{
        'w-full sm:w-[230px]': expanded,
        'w-full sm:w-12': !expanded,
      }"
    >
...
const moduleToEdit = ref<{ _id: string; title: string; icon: any } | null>(
  null,
);
