<template>
  <div class="relative">
    ...
    <button
      ...
      class="relative flex items-center justify-center mt-2  rounded-full cursor-pointer"
    >
    ...
    <div v-if="isOpen"
      ...
      <div class="px-4 py-3 border-b border-border-input  flex items-center justify-between">
        ...
        <button v-if="notifications.length" class="text-[12px] cursor-pointer font-bold font-manrope text-accent underline"
                @click="markAllRead">Mark all as read</button>
      </div>

      <!-- Loading / Error -->
      <div v-if="notificationsQuery.isLoading.value" class="p-5 text-sm text-gray-500">Loading...</div>
      <div v-else-if="notificationsQuery.isError.value" class="p-5 text-sm text-red-500">Failed to load notifications</div>
          ...
          <div v-for="notification in group" :key="notification.id"
            ...
            <div class="flex-1 min-w-0">
              <p class="text-sm truncate"
                ...
                <span class="text-text-secondary text-[14px] font-normal font-manrope block">
                  {{ notification.action || notification.title }}
                </span>
              </p>
              <p class="text-xs  text-text-secondary mt-1 truncate font-manrope">{{ timeAgo(notification.created_at) }}</p>
            </div>
