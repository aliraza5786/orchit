...
watchEffect(() => {
  ...
  nextTick(() => {
    ...
    instance.bus.addListener("operation", async (data: any) => {
      ...
      if (
        !data ||
        (data.name !== "insertSibling" && data.name !== "addChild")
      ) {
        return;
      }
      ...
      // Determine parent node correctly
      const parentNode =
        data.name === "addChild"
          ? instance.currentNode?.nodeObj
          : instance.currentNode?.nodeObj?.parent;

      if (!parentNode) return;

      // Only allow cards under List
      if (
        !("unique_name" in parentNode) ||
        (parentNode as any).unique_name !== "List"
      ) {
        return;
      }
