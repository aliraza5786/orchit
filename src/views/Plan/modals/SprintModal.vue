<template>
  <BaseModal v-model="isOpen" modalClass="!py-0" size="md">
    ...
    <div class="sticky top-0 z-10 flex flex-col items-start pt-6 px-6 border-b border-border bg-bg-body pb-4 mb-4">
      <h2 class="text-xl font-semibold">Add Sprint</h2>
    ...
    <div class="px-6  gap-4">
      <!-- Name (required) -->
      <BaseTextField v-model="form.name" label="Name" placeholder="Sprint 1" :error="!!nameError" :message="nameError"
    ...
    <div class="flex justify-end gap-2 p-6 mt-8 sticky bottom-0 bg-bg-body border-t border-border">
      ...
      <Button variant="primary" :disabled="!isValid" @click="save">{{ creatingSprint ? 'Saving...' : 'Save' }}</Button>
    </div>
  </BaseModal>
</template>
