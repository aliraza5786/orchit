...
watch(normalizedBacklog, (tickets) => {
  ...
  selectedBacklogIds.value = selectedBacklogIds.value.filter((id) => validIds.has(id))
})
...
function onDragStart(e: DragEvent, ticket: Ticket, from: 'backlog' | 'sprint') {
  ...
  const payload = JSON.stringify({ id: ticket.id, ids: selection, from, tickets })
...
function onDragEnd(e: DragEvent) {
  ...
  if (e.dataTransfer?.dropEffect !== 'none' && draggedTicketIds.value.length) {
    normalizedBacklog.value = normalizedBacklog.value.filter(t => !draggedTicketIds.value.includes(t.id))
    selectedBacklogIds.value = selectedBacklogIds.value.filter(id => !draggedTicketIds.value.includes(id))
  }
...
function onDropBacklog(e: DragEvent) {
  ...
  try {
    ...
    if (!raw) return
    const data = JSON.parse(raw) as { id?: string; ids?: string[]; from: 'backlog' | 'sprint'; sprintId?: string; ticket?: Ticket; tickets?: Ticket[] }
    ...
    const incomingTickets = (data.tickets && data.tickets.length
      ...
      : data.ticket
        ? [data.ticket]
        : []).filter(Boolean) as Ticket[]
    const deduped = incomingTickets.filter(ticket => !normalizedBacklog.value.some(t => t.id === ticket.id))
    if (deduped.length) normalizedBacklog.value.push(...deduped)
    const ids = data.ids && data.ids.length ? data.ids : data.id ? [data.id] : []
...
function toggleRowSelection(id: string, checked: boolean) {
  if (checked) {
    if (!selectedBacklogIds.value.includes(id)) selectedBacklogIds.value.push(id)
  } else {
    selectedBacklogIds.value = selectedBacklogIds.value.filter(existing => existing !== id)
  }
}
