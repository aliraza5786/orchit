<template>
    <BaseModal v-model="isOpen" size="sm">
        ...
        <div class="px-6 py-4 flex flex-col gap-4">
            <!-- Dropdown Title -->
            <BaseTextField v-model="dropdownTitle" label="Dropdown Title"
                placeholder="Enter dropdown title (max 30 chars)" :error="titleError ? true : false" maxlength="30"
                @input="validateDropdownTitle" />
                <BaseTextField v-model="description" label="Option Description"
                placeholder="Optional description" />
            <!-- Option Input -->
            <div class="flex gap-2 items-start">
                <div class="flex-1 flex flex-col gap-1">
                    <BaseTextField v-model="optionTitle" label="Option Title" placeholder="Option title (max 30 chars)"
                        :error="optionTitleError ? true : false" maxlength="30" @input="validateOptionTitle" />
            ...
            <ul v-if="options.length" class="mt-2 border border-border rounded-md p-2 bg-bg-input max-h-48 overflow-auto">
...
const emit = defineEmits<{
    ...
    // (e: 'created', payload: { title: string; options: { title: string; description?: string }[] }): void
}>()
...
const isValid = computed(() => {
    validateDropdownTitle()
    return !titleError.value && options.value.length > 0
})
...
function submit() {
    ...
    const payload = {
        ...
        title: dropdownTitle.value.trim(), data: options.value,
