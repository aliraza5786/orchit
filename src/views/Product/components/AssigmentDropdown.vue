<template>
  <div class="relative" @click.stop @keydown.esc="close">
    <!-- Trigger -->
    <template v-if="assigneeId || seat?._id">
      <img @click="toggle" v-if="assigneeId?.avatar_url" :src="assigneeId?.avatar_url" class="w-6 h-6 rounded-full"
        alt="" />
      <div v-else @click="toggle"
        ...
        :style="{ backgroundColor: colorFor(assigneeId ?? seat) }">
        {{ initials(assigneeId?.u_full_name ?? seat?.title) }}
      </div>
    </template>
    <button v-else type="button" class="inline-flex items-center gap-2 rounded-full border border-border  px-2 py-1 text-xs
    ...
    <div v-if="open" class="absolute right-0 z-50 mt-2 w-60 rounded-md border border-border bg-bg-dropdown shadow-xl"
      @click.stop>
      <div class="p-3">
        <input ref="searchRef" v-model="query" type="text" placeholder="Add people by name or email" class="w-full rounded-md border border-border h-[30px]  bg-bg-input  px-3  outline-none
                   text-xs" />
      </div>
      ...
      <ul class="max-h-80 overflow-auto py-1">
        <li v-for="u in filteredUsers" :key="u._id"
          ...
          <div class="shrink-0">
            <button v-if="  seat?._id == u?._id"
              class="text-xs cursor-pointer text-red-400 hover:underline" @click.stop="unassign()">
              UnAssign
            </button>
            <button v-else class="text-xs text-[#2563eb] cursor-pointer hover:underline" @click.stop="assign(u._id)">
              Assign
            </button>
        ...
        <li v-if="!filteredUsers.length" class="px-4 pb-2 text-sm text-text-secondary -500 text-secondary-400">
          No matches
        </li>
      </ul>
    </div>
