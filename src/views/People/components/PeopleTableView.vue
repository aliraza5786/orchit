<template>
  <div
    class="flex flex-col h-[calc(100vh-125px)] relative rounded-[6px] mt-2 overflow-hidden border border-border/60"
  >
    <div
      ref="tableViewport"
          ...
          >
            ...
            >
              <td
                ...
                class="p-2 text-sm font-semibold text-text-primary border-r-0"
              >
                <div class="flex items-center gap-2">
                  ...
                  <span class="text-text-secondary font-normal text-[11px] ml-2"
                    >{{
                      group.cards?.length || group.agents?.length || 0
                    }}
                    seats</span
                  >
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        <i
                          v-if="item.icon"
                          :class="`${item.icon.prefix} ${item.icon.iconName}`"
                        ></i>
                ...
                >
                  ...
                  >
                    <div class="text-[12px] text-text-secondary truncate">
                      {{
                        person[col.key] ||
                        getVariableValue(person, col.key) ||
                        "-"
                      }}
                    </div>
                  </slot>
...
const { canInviteUser, canEditUser, canDeleteUser, canCreateVariable } =
  usePermissions();
...
const getVariableValue = (person: any, colKey: string) => {
  ...
  const v = values.find(
    (v: any) =>
      v.module_variable_id === colKey ||
      v._id === colKey ||
      v.variable_id === colKey,
  );
  return v?.value || null;
};
...
const RenderCell = (p: { row: any; column: any; index: any }) => {
  ...
  return h(
    "span",
    String(p.row[p.column.key] || getVariableValue(p.row, p.column.key) || "-"),
  );
};
