<template>
  <BaseModal v-model="isOpen" size="md" modalClass="!py-0">
    ...
    <div class="px-6 flex flex-col gap-4">
      ...
      <BaseSelectField size="md" label="Permission" :options="roleOptions" placeholder="Choose role"
        :model-value="form.role_id" @update:modelValue="onRoleChange" 
...
import { useWorkspaces, useSingleWorkspaceCompany } from '../../../queries/useWorkspace'
...
// --- Roles Loading Logic ---
const workspaceIdRef = computed(() => form.workspace_id ?? undefined) as Ref<string | number>;
...
const { data: workspaceRoles } = useWorkspaceRoles(newCompanyId, {
  enabled: computed(() => !!newCompanyId.value),
});
...
const selectedRoleData = computed(() => {
  if (!workspaceRoles.value || !form.role_id) return null;
  return workspaceRoles.value.find((r: any) => r._id === form.role_id);
});
...
const selectedRole = computed(() => {
  return workspaceRoles.value?.find((r:any) => r._id === form.role_id) || null;
});

// format the title of the role
function cleanRoleTitle(title: string) {
  if (!title) return title;
  return title.replace(/workspace/i, "").trim().toLowerCase();
}
...
function submit() {
  ...
  invitePeople(
    {
      payload: {
        company_id: company_id.value?._id ?? newCompanyId.value, // Fallback to fetching via workspace if global company_id fails
        workspace_id: form.workspace_id,
        role: cleanRoleTitle(selectedRole.value?.title || ""), // cleaned title
        company_role_id: form.role_id, // role id
