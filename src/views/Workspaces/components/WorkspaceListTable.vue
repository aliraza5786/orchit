...
import { useDeleteWorkspace, useArchiveWorkspace, useWorkspaceModulesAndUsers } from '../../../queries/useWorkspace'
...
interface Row {
  ...
  owner?: { name: string; profile_img?: string }
  task_stats?: { total_percentage?: number }
  has_permission_to_manage_user?: boolean
  People?: any[]
  company?: {
    _id: string
    title: string
...
const tableRows = computed(() =>
  isLoading.value ? skeletonRows.value : paginatedItems.value
)
const totalCount = computed(() => items.value.length)

// Reset to page 1 when filter or data changes
watch(() => props.filter, () => { page.value = 1 })
watch(() => props.projects, () => { page.value = 1 })
...
const { mutate: archiveWorkspace } = useArchiveWorkspace({
  onSuccess: () => {
    const msg = props.filter === 'archived'
...
const handleArchive     = (row: any) => archiveWorkspace({ id: row._id })
const openDeleteConfirm = (row: any) => { workspaceToAction.value = row; showDeleteConfirm.value = true }
...
const renderOrganization = ({ row }: any) => {
  ...
  const handleRedirect = (e: Event) => {
    ...
    const base = company.domain_link.replace(/\/$/, '') // remove trailing slash
  ...
  return h('div', {
    ...
    company.logo
      ...
      : h('div', {
          class: 'h-8 w-8 rounded-full bg-bg-card flex items-center justify-center text-xs font-medium',
        }, company.title?.charAt(0)?.toUpperCase() || '?'),

    h('span', { class: 'truncate' }, company.title),
  ])
}
/* ------------ Render functions ------------ */
const renderActions = ({ row }: any) => {
  ...
  return h('div', { class: 'flex items-center gap-1 justify-end opacity-0 group-hover:opacity-100 transition-opacity' }, [
    h('button', {
      ...
      onClick: (e: Event) => { e.stopPropagation(); handleClick({ row }) },
      title: 'View',
    }, [h('i', { class: 'fa-regular fa-eye text-sm' })]),

    row.has_permission_to_manage_user ? h('button', {
      class: 'p-2 hover:bg-bg-body rounded-md transition-colors text-text-secondary hover:text-text-primary',
      onClick: (e: Event) => { e.stopPropagation(); openShareModal(row) },
      title: 'Share',
    }, [h('i', { class: 'fa-regular fa-share-nodes text-sm' })]) : null,

    row.has_permission_to_manage_user ? h('button', {
      class: 'p-2 hover:bg-bg-body rounded-md transition-colors text-text-secondary hover:text-text-primary',
      onClick: (e: Event) => { e.stopPropagation(); handleArchive(row) },
      title: archiveTitle,
    }, [h('i', { class: `${archiveIcon} text-sm` })]) : null,

    row.has_permission_to_manage_user ? h('button', {
      class: 'p-2 hover:bg-red-500/10 rounded-md transition-colors text-red-500 hover:text-red-600',
      onClick: (e: Event) => { e.stopPropagation(); openDeleteConfirm(row) },
      title: 'Delete',
    }, [h('i', { class: 'fa-regular fa-trash-can text-sm' })]) : null,
  ].filter(Boolean))
}
...
const renderProject = ({ row, value }: any) =>
  h('div', { class: 'flex items-center gap-2' }, [
    row.logo
      ? h('img', { src: row.logo, alt: value?.title || 'Project', class: 'h-8 w-8 bg-bg-card rounded-full object-cover', loading: 'lazy', decoding: 'async' })
      : h('div', { class: 'h-8 w-8 rounded-full bg-bg-card' }),
    h('span', { class: 'cursor-pointer hover:underline', onClick: (e: Event) => { e.stopPropagation(); handleClick({ row }) } }, value?.title || 'Untitled'),
  ])
...
const renderCompanyPercentage = ({ row }: any) => {
  ...
  return h('div', { class: 'flex items-center gap-2 w-full' }, [
    h('div', { class: 'flex-1 h-2 rounded-full bg-bg-body border border-border overflow-hidden' }, [
      h('div', { class: 'h-full rounded-full bg-accent transition-all duration-300', style: { width: `${percentage}%` } }),
    ]),
    h('span', { class: 'text-xs text-text-secondary w-7 text-right' }, `${percentage}%`),
  ])
}

const renderProjectType = ({ value }: any) => {
  const type = value?.['workspace-type']

  const labelMap: Record<string, string> = {
    team: 'Organization',
  }

  return h('span', { class: 'capitalize' }, labelMap[type] || type || '-')
}
  ...
  h('div', { class: 'flex items-center -space-x-3' }, [
    ...
    h('button', {
      class: row.has_permission_to_manage_user
        ? 'flex justify-center items-center rounded-full border border-border text-xs bg-bg-dropdown cursor-pointer hover:bg-bg-dropdown-menu-hover transition h-8 w-8'
        : 'hidden',
      onClick: (e: Event) => { e.stopPropagation(); openInviteModal(row._id) },
      title: 'Invite Users',
...
const renderCompanyAdmin = ({ row }: any) => {
  ...
  return h('div', { class: 'flex items-center gap-2' }, [
    owner.profile_img
      ? h('img', { src: owner.profile_img, alt: owner.name, class: 'h-6 w-6 rounded-full object-cover', loading: 'lazy', decoding: 'async' })
      : h('div', { class: 'h-8 w-8 border-primary border-2 shadow-md flex items-center justify-center rounded-full text-white text-xs font-medium', style: { backgroundColor: getColorFromString(owner.name) } }, getInitials(owner.name)),
    h('span', owner.name),
  ])
}
...
<template>
  ...
  <Table
    v-if="isLoading || paginatedItems.length"
    :columns="columns"
    :rows="tableRows"
    :loading="isLoading"
    :total="totalCount"
