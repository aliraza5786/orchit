  ...
  <Teleport to="body">
    <Transition name="modal" appear>
      <div v-if="modelValue" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="closeModal"
        ...
        <div class="relative bg-gradient-to-b from-bg-card/95 to-bg-card/90 backdrop-blur
                    rounded-2xl shadow-[0_20px_60px_-10px_rgba(0,0,0,.7)]
                    border border-orchit-white/10 w-full max-w-4xl max-h-[90vh] overflow-hidden
          ...
          <template v-else-if="cardDetails">
            <div
              class="sticky top-0 z-10 bg-bg-card/90 backdrop-blur border-b border-orchit-white/5 px-6 py-4 flex items-center justify-between">
              ...
              <div class="flex flex-col gap-5">
                <div class="capitalize">
                  <Transition name="fade-scale" mode="out-in">
                    <input v-if="editingTitle" key="title-edit" ref="titleInput" v-model="localTitle"
                      @blur="saveTitle" @keydown.enter.prevent="saveTitle" @keydown.esc.prevent="cancelEdit" class="w-full text-2xl font-semibold rounded-xl px-3 py-2 bg-orchit-white/5 border border-orchit-white/10
                  ...
                  <section v-if="activeTab === 'details'" key="tab-details" class="space-y-6">
                    ...
                    <div
                      ...
                      <div class="space-y-2">
                        ...
                        <BaseSelectField size="sm" :options="laneOptions" placeholder="Select lane"
                          :allowCustom="false" :model-value="lane" @update:modelValue="setLane" />
                      </div>
                      <div class="space-y-2">
                        <div class="text-xs uppercase tracking-wider text-text-secondary">Assign</div>
                        <AssigmentDropdown @assign="assignHandle" :assigneeId="curentAssigne"
                          :seat="details.seat" />
                      </div>
                      <template v-if="!pin">
                        <div class="space-y-2">
                      ...
                      <template v-if="cardDetails?.variables"
                        v-for="(item, index) in cardDetails?.variables" :key="item.slug || `var-${index}`">
                        <div v-if="item?.type === 'Select'" class="space-y-2 sm:col-span-1">
                          ...
                          <BaseSelectField size="sm"
                            :options="item?.data.map((e: any) => ({ _id: e, title: e }))"
                            placeholder="Select option" :allowCustom="false"
                    ...
                    <div v-for="c in (comments ?? [])" :key="c._id"
                      ...
                      <div v-if="c?.attachments?.length" class="mt-3 grid grid-cols-2 gap-2">
                        <a v-for="(file, index) in c.attachments" :key="index" :href="file.url" target="_blank" class="group flex items-center gap-2 rounded-lg border border-orchit-white/10 bg-orchit-white/5 px-2 py-1
                          ...
                          <i
                            class="fa-regular fa-file text-text-secondary group-hover:text-text-primary transition"></i>
...
const { data: cardDetails, isLoading } = useProductCard(computed(() => props.cardId), {
