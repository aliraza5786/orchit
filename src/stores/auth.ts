...
export const useAuthStore = defineStore('auth', {
  state: () => {
    ...
    // Root domain → never hydrate company
    if (onRoot) {
      return {
        user: null as any,
        initialized: false,
    ...
    // Subdomain → restore company from cookie/localStorage
    return {
      user: null as any,
      initialized: false,
      userId: localStorage.getItem('user_id') as string | null,
  ...
  actions: {
    ...
    async bootstrap() {
      ...
      // Persist token to cookie if needed
      ...
      if (isRootDomain()) {
        // Root domain always clears tenant context
        ...
        console.log(
          '🏢 Company restored from session:',
          session.company_id
        )
      } else {
        // Fallback for subdomain reloads
        ...
        if (storedCompanyId) {
          ...
          console.log(
            '🏢 Company restored from localStorage:',
            storedCompanyId
          )
        }
      }
      ...
      try {
        ...
        const activeCompanyId =
          res.data?.data?.active_company_id

        console.log(
          'active company id in store',
          activeCompanyId
        )
        ...
        if (
          ...
          console.log(
            '🏢 Active company auto-set from profile:',
            activeCompanyId
          )
        }
      } catch (e) {
